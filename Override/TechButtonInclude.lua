...
function AddSmallButtonsToTechButton( thisTechButtonInstance, tech, maxSmallButtons, textureSize )
	...
	if tech.WorkerSpeedModifier ~= 0 then
		...
		if thisButton then
			IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
			thisButton:SetHide( false );
			thisButton:SetToolTipString( Locale.ConvertTextKey( "TXT_KEY_TECH_WORKER_RATE", tech.WorkerSpeedModifier ) );
			buttonNum = buttonNum + 1;
		end
	end

	-- This one is hardcoded, sorry :/
	if tech.Type == "TECH_NETWORKS" then
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
			thisButton:SetHide( false );
			thisButton:SetToolTipString( Locale.ConvertTextKey( "TXT_KEY_TECH_NETWORK_YIELDS" ) );
			buttonNum = buttonNum + 1;
		end
	end
